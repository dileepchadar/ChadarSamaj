    ...
    <nav className="bg-white shadow-md relative z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link to="/" className="text-2xl font-bold text-primary" onClick={closeMenu}>Simple Matrimony</Link>
          </div>
          
          {/* Desktop Menu */}
          <div className="hidden md:flex items-center space-x-4">
            <Link to="/" className="text-gray-700 hover:text-primary font-medium px-3 py-2">{t('home')}</Link>
            <Link to="/search" className="text-gray-700 hover:text-primary font-medium px-3 py-2">{t('allMembers')}</Link>
            ...
            {user ? (
              ...
              <>
                <Link to="/login" className="text-gray-700 hover:text-primary font-medium px-3 py-2">{t('login')}</Link>
                <Link to="/register" className="btn btn-primary text-sm py-2 px-4 shadow-md transform transition hover:scale-105">{t('register')}</Link>
              </>
            )}
          </div>
      ...
      {isOpen && (
          <div className="md:hidden bg-white border-t border-gray-100 absolute w-full left-0 shadow-xl flex flex-col p-4 space-y-3 z-50 animate-fade-in-down">
            <Link to="/" onClick={closeMenu} className="text-gray-700 hover:text-primary font-medium px-3 py-2 rounded hover:bg-gray-50">{t('home')}</Link>
            <Link to="/search" onClick={closeMenu} className="text-gray-700 hover:text-primary font-medium px-3 py-2 rounded hover:bg-gray-50">{t('allMembers')}</Link>
            ...
            {user ? (
              ...
              <>
                <Link to="/login" onClick={closeMenu} className="text-gray-700 hover:text-primary font-medium px-3 py-2 rounded hover:bg-gray-50">{t('login')}</Link>
                <Link to="/register" onClick={closeMenu} className="bg-primary text-white text-center font-bold py-3 rounded-lg shadow-lg">{t('register')}</Link>
              </>
            )}
          </div>
      )}
